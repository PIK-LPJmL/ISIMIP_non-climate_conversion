...
# Time series can be split into multiple files in ISIMIP source data.
scenario_years <- list(
  "2015soc" = list(1601:1849, 1850:2014,2015:2100),
  histsoc = 1850:2014,
  "1850soc" = 1601:1849,
  "ssp126soc-adapt" = 2015:2100,
  "ssp126soc-noadapt" = 2015:2100,
  "ssp370soc-adapt" = 2015:2100,
  "ssp370soc-noadapt" = 2015:2100,
  "ssp585soc-adapt" = 2015:2100,
  "ssp585soc-noadapt" = 2015:2100
)
scenario_gcm_specific <- list(
  "2015soc" = FALSE,
  histsoc = FALSE,
  "1850soc" = FALSE,
  "ssp126soc-adapt" = FALSE,
  "ssp126soc-noadapt" = FALSE,
  "ssp370soc-adapt" = FALSE,
  "ssp370soc-noadapt" = FALSE,
  "ssp585soc-adapt" = FALSE,
  "ssp585soc-noadapt" = FALSE
)
...
output_scenarios <- list(
  ...
  histsoc = "histsoc",
  "1850soc" = "1850soc",
  "ssp126soc-adapt" = "ssp126soc-adapt",
  "ssp126soc-noadapt" = "ssp126soc-noadapt",
  "ssp370soc-adapt" = "ssp370soc-adapt",
  "ssp370soc-noadapt" = "ssp370soc-noadapt",
  "ssp585soc-adapt" = "ssp585soc-adapt",
  "ssp585soc-noadapt" = "ssp585soc-noadapt"
)
...
# GCMs not relevant, because all ISIMIP deposition inputs are independent of
# GCM.
GCMs <- c(
  ...
  for (gcm in switch(
    ...
    for (scen in output_scenarios[[output]]) {
      if (is.list(scenario_years[[scen]])) {
        ...
        if (
          !any(file.exists(inputname_no3)) && !any(file.exists(inputname_nh4)) &&
            grepl("ssp", scen)
        ) {
          socscen <- regmatches(scen, regexec("ssp[0-9]+soc", scen))
          print(
            paste(
              "Warning: Using", sQuote(socscen, q = FALSE), "instead of",
              sQuote(scen, q = FALSE), "as scenario"
            )
          )
          inputname_no3 <- file.path(
            isimip_source_dir, scen,
            paste0(
              "ndep-noy_", socscen, "_monthly_",
              sapply(scenario_years[[scen]], min), "_",
              sapply(scenario_years[[scen]], max),
              ".nc"
            )
    ...
    filename_no3 <- file.path(
      output_dir,
      paste0(
        "no3-deposition_isimip3b_", output, "_",
    ...
    filename_nh4 <- file.path(
      output_dir,
      paste0(
        "nh4-deposition_isimip3b_", output, "_",
        ifelse(output_gcm_specific[[output]], paste0(gcm, "_"), ""),
        fileyears[1], "-", fileyears[2], ".clm"
      )
    )
